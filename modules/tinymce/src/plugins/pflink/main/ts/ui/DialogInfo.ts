...
const extractFromAnchor = (editor: Editor, anchor: HTMLAnchorElement) => {
  ...
  const tourId = nonEmptyAttr(dom, anchor, 'data-pfbind-start-tour');
...
const collect = (editor: Editor, linkNode: HTMLAnchorElement): Promise<LinkDialogInfo> => (ToursListOptions.getLinks(editor)).then((tours) => {
