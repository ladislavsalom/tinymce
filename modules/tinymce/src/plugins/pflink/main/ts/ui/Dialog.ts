...
const getInitialData = (info: LinkDialogInfo, defaultTarget: Optional<string>): LinkDialogData => {
  ...
  return {
    ...
    tourId: (info.optNode.getOrNull()?.getAttribute('data-pfbind-start-tour')) || ''
  };
};
...
const makeDialog = (settings: LinkDialogInfo, onSubmit, editor: Editor): Dialog.DialogSpec<LinkDialogData> => {
  ...
  const body: Dialog.TabPanelSpec = {
    ...
    },
    {
      ...
      items: Arr.flatten<Dialog.BodyComponentSpec>([
        Optionals.cat([
          catalogs.tours.map(ListOptions.createUi('tourId', 'Tour')),
          catalogs.classes.map(ListOptions.createUi('linkClass', 'Display Type'))
        ])
      ])
    }
