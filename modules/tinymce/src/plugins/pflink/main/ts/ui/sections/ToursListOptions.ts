...
const getLinks = (editor): Promise<Optional<ListItem[]>> => {
  ...
  return new Promise<Optional<UserListItem[]>>((callback) => {
    XHR.send({
      ...
      url: '/api/v1/tours/' + window['auth'].project.UniqueCode, // 'https://app.productfruits.com/api/v1/tours/yiRkZaruvpA-H2sy',
    ...
    return Optional.from(noneItem.concat(data.items.map((i) => {
      return {
        text: i.name,
        value: i.id.toString()
      };
